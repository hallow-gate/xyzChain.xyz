...
class Mnemonic
{
    ...
    private static $entropyBits = 256;
    private static $checksumBits = 8;
    private static $totalBits = 264;
    private static $wordCount = 24;
    
    public static function generate()
    {
        $entropy = random_bytes(self::$entropyBits / 8);
        $checksum = substr(hash('sha256', $entropy, true), 0, self::$checksumBits / 8);
        $combined = $entropy . $checksum;
        
        $words = [];
        $binary = '';
        for ($i = 0; $i < strlen($combined); $i++) {
            $binary .= str_pad(decbin(ord($combined[$i])), 8, '0', STR_PAD_LEFT);
        }
        
        for ($i = 0; $i < self::$totalBits; $i += 11) {
            $index = bindec(substr($binary, $i, 11));
    ...
    public static function validate($mnemonic)
    {
        $words = explode(' ', trim($mnemonic));
        
        if (count($words) !== self::$wordCount) {
            return false;
        }
    ...
    public static function toSeed($mnemonic, $passphrase = '')
    {
